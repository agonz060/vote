<?php
	...
	function getComments($poll_id,$pollType,$actionNum=0,$confidentialEvals) {
		...
		if($confidentialEvals == $FALSE && $multiActionPollType) { // multiaction poll types
			...
			if($confidentialEvals) {
				...
				$stmt = "SELECT voteCmt FROM $dataTable INNER JOIN Users on Users.user_id=assistant_data.user_id WHERE $evaluationRestrictions assistant_data.poll_id=$poll_id"; 
			} else {
	...
	function checkForEvaluations($poll_id,$multiActionPollType,$actionNum) {
		...
		if($evaluationRestrictions) {
			...
			$stmt = "SELECT count(assistant_data.voteCmt) AS commentCount FROM assistant_data INNER JOIN Users on Users.user_id=assistant_data.user_id WHERE $evaluationRestrictions assistant_data.poll_id=$poll_id";
			$result = $conn->query($stmt) or die($conn->error);
			$row = $result->fetch_assoc();
