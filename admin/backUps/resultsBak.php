<?php
	...
	function getProfessorFormOptions($pollID) {
		...
		$query = "SELECT assistantForm, assistantEvaluationNum, associateForm, fullForm FROM Polls WHERE poll_id=$pollID";
	...
	function getVotingOptions($pollType,$otherVotingOptionsSet=0) {
		...
		$options = array("Merit_Data"=>$inFavorOpposedAbstain,
							"Associate_Promotion_Data" => $inFavorOpposedAbstain,
							"Reappointment_Data" => $inFavorOpposedAbstain,
							"Fifth_Year_Review_Data" =>$satisfactoryWQualifications,
							"Fifth_Year_Appraisal_Data" => $positiveOpposedAbstain
						 );
		$key = pollTypeToTable($pollType);
		return $options[$key];
	}
	function printReviewComments($comments,&$format) {
		...
		for($x=0; $x < count($comments); $x++) {
			...
			$qualificationsCmt = $qualificationsCmt.trim();
	...
	function printAppraisalComments($comments,&$format) {
		...
		for($x=0; $x < count($comments); $x++) {
			...
			$researchCmts = $researchCmts.trim();
			$teachingCmts = $teachingCmts.trim();
			$pubServiceCmts = $pubServiceCmts.trim();
	...
	function getComments($poll_id,$pollType,$confidentialEvals) {
		# Variables
		global $conn;
		$FALSE = 0;
		$COMMENT = "voteCmt";
		$comments = array();
		# Get data table name
		$dataTable = pollTypeToTable($pollType);
		if($confidentialEvals) {
			$dataTable = "assistant_data";
		}
		# Get comments
		//print "CE: $confidentialEvals";
		if($confidentialEvals == $FALSE && ($pollType == 'Merit' || $pollType == 'Promotion' || $pollType == 'Other')) { // multiaction poll types
	...
	function checkForEvaluations($poll_id) {
		global $conn;
		$EVALUATION = 3;
		// $formOptions[$key]; keys: assistant, assistantEvaluationNum, associate, full
		$formOptions =getProfessorFormOptions($poll_id);
		//print_r($formOptions); print "<hr>";
		$selection = null;
		$eligibleCount = $actionEvaluationNum = 0;
		...
		$stmt = "SELECT assistantEvaluationNum FROM Polls WHERE poll_id=$poll_id";
		$result = $conn->query($stmt) or die($conn->error);
		$row = $result->fetch_assoc();
		$actionEvaluationNum = $row['assistantEvaluationNum'];

		// Select all members of the current vote that have been selected to place comments, but not to cast a vote
		$stmt = "SELECT count(Voters.user_id) as Eligible FROM Voters INNER JOIN Users on Users.user_id=Voters.user_id WHERE $selection Voters.poll_id=$poll_id";
		$result = $conn->query($stmt) or die($conn->error);
		$row = $result->fetch_assoc();
		...
		$evaluationInfo = array('evaluationNum' => $actionEvaluationNum,
	...
	function printActionResults($description,$actionNum,$format,&$pollData,$confidentialEvals=0) {
		...
		if($confidentialEvals) {
			$evaluationInfo = checkForEvaluations($pollData['poll_id']);
			$evaluationActionNum = $evaluationInfo['evaluationNum'];
			$eligibleEvalsCount = $evaluationInfo['eligible'];
			if($evaluationActionNum == $actionNum && $eligibleEvalsCount > 0) {
	...
	function printComments($poll_id,$pollType,$actionNum,&$format,$confidentialEvals) {
		$comments = getComments($poll_id,$pollType,$confidentialEvals);
		$actionNum = (string)$actionNum;
		if($confidentialEvals) {
